...
export default function AgentConsole() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      reload();
    } catch (err) { toast.error(err?.response?.data?.detail || '生成失败', 6000); }
  ...
  const statusCards = [
    ...
    { label: 'LLM 状态', value: statusData?.llm_status || 'unknown', hint: statusData?.llm_provider || '' },
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.metricsRow}>
        {statusCards.map((s) => <MetricCard key={s.label} label={s.label} value={s.value} unit={s.unit || ''} hint={s.hint} status="info" />)}
      </div>

      <div className={styles.sections}>
        <SectionCard title="Agent 列表">
          <AsyncState loading={loading} error={error} onRetry={reload}>
            <Table columns={agentColumns} rows={agentList} rowKey="name" />
          </AsyncState>
        </SectionCard>

        <SectionCard title="章节生成">
          <div className={styles.formRow}>
            <select className={styles.select} value={selectedProjectId} onChange={(e) => setSelectedProjectId(e.target.value)} disabled={loadingProjects}>
              <option value="">-- 选择项目 --</option>
              {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <input type="number" className={styles.input} value={selectedChapterIndex} onChange={(e) => setSelectedChapterIndex(Number(e.target.value))} min={1} />
      ...
      <Modal open={!!runDetailId} onClose={() => setRunDetailId(null)} title={runDetailData?.user_request || '运行详情'}>
        {loadingDetail ? <div className={styles.detailSkeleton} /> : runDetailData ? (
