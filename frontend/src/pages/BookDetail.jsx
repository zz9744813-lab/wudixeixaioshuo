  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <Link to="/books" className={styles.back}>← 返回书籍列表</Link>
        <h1 className={styles.title}><Icon name={PAGE_ICON} size={22} /><span>{book?.title || PAGE_TITLE}</span>
        ...
        <div className={styles.summary}>
          ...
          <span>章节：{book?.total_chapters || 0}</span>
          <span>字数：{(book?.total_words || 0).toLocaleString()}</span>
        </div>
      </header>

      {error && !book && <p className={styles.err}>{error}</p>}

      {book && (
        <>
          <div className={styles.statsGrid}>
            <StatCard label="总章节" value={book.total_chapters || 0} />
            <StatCard label="总字数" value={(book.total_words || 0).toLocaleString()} unit="字" />
            <StatCard label="分析进度" value={`${book.analysis_progress || 0}%`} />
          </div>
            ...
            {activeTab === 'analysis' && (
              <div className={styles.metaGrid}>
                <div className={styles.analysisSection}>
                  <h3 className={styles.analysisTitle}>章节概览</h3>
                  {chapters.length === 0 ? <p className={styles.empty}>暂无数据</p> : (
                    <div className={styles.chapterList}>
                ...
                <div className={styles.analysisSection}>
                  <h3 className={styles.analysisTitle}>人物 / 剧情点 / 钩子</h3>
